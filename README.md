...
A `BlackHole::Sun` graph is a hierarchical, static-topology DAG where each node is an agent implemented in terms of `jungle::Flow` that progress through 3 phases:
...
`black-hole-sun` is intended to be a research tool and art project. Documentation is sparse and the interface should be considered highly unstable.
