...
`black-hole-sun` is an extension of the [Jungle](https://github.com/nicksenger/Jungle) "workflow-as-type" (WAT) orchestration system that performs blackbox co-optimization of distributed artificial intelligence agents.

It runs on a [custom engine](https://github.com/nicksenger/paramecia) which supports GGUF quantizations of the Qwen3.* architecture (e.g. Qwen3.8 27b, Qwen3-Next, etc), and implements the forward-only optimization strategies described in these publications:
...
Nodes in a `BlackHole::Sun` graph (called `Cell`s) are agents implemented in terms of `jungle::Flow` which progress through 3 phases:
...
The role of `black-hole-sun` is to orchestrate this cycle efficiently and reliably over arbitrarily large, hierarchical networks of cells.

`black-hole-sun` is an art project and research tool. Documentation is sparse and the interface is highly unstable.
