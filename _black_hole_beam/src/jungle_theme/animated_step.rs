//! Port of jungle-vision's `AnimatedStepNode` widget.
//!
//! Logic is identical to the upstream widget; the green-tinted colors are
//! hue-rotated to blue so step nodes match beam's blue palette.
    ...
    Message: Clone + 'static,
{
    ...
    // Signature mirrors the upstream widget, which also takes eight arguments.
    ...
    fn as_element(&self, fill: Color) -> Element<'_, Message> {
        ...
        let body = column![
            ...
            text(self.label.as_str())
                .size(13)
                // Bluish white; upstream uses the green-tinted (223, 245, 230).
                .color(Color::from_rgb8(223, 228, 245))
        ]
        .spacing(4);
            ...
            .style(move |_theme, status| {
                let (styled_fill, border_color, border_width, shadow) = match status {
                    button::Status::Hovered => (
                        // Upstream mixes toward the dark green (20, 74, 45).
                        mix_color(fill, Color::from_rgb8(33, 57, 123), 0.62),
                        Color::from_rgb8(227, 206, 96),
                        2.2,
                        iced::Shadow {
                            // Upstream shadow is the green-tinted (7, 23, 14).
                            color: Color::from_rgba8(12, 18, 38, 0.55),
                            offset: iced::Vector::new(0.0, 4.0),
                            blur_radius: 9.0,
                        },
                    ),
                    button::Status::Pressed => (
                        // Upstream mixes toward the dark green (11, 52, 31).
                        mix_color(fill, Color::from_rgb8(18, 35, 87), 0.8),
                        Color::from_rgb8(237, 219, 125),
                        2.8,
                        iced::Shadow {
                            // Upstream shadow is the green-tinted (5, 17, 10).
                            color: Color::from_rgba8(8, 13, 28, 0.68),
                    ...
                    _ => (
                        ...
                        iced::Shadow {
                            // Upstream shadow is the green-tinted (7, 23, 14).
                            color: Color::from_rgba8(12, 18, 38, 0.25),
                ...
                iced::widget::button::Style {
                    ...
                    text_color: Color::from_rgb8(223, 228, 245),
        ...
        if let Event::Mouse(iced::mouse::Event::ButtonPressed(iced::mouse::Button::Left)) = event {
            if cursor.is_over(layout.bounds()) {
                info!(
                    ...
                    "black-hole-beam jungle step node clicked"
                );
            }
        }
