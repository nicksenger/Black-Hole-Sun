//! Port of jungle-vision's `AnimatedClusterView` widget.
//!
//! Logic is identical to the upstream widget; the green-tinted label chip
//! background is hue-rotated to blue so clusters match beam's blue palette.
...
static CLUSTER_TWEEN_CACHE: OnceLock<Mutex<HashMap<u64, AnimatedClusterState>>> =
    OnceLock::new();
            ...
            Mode::Overlay => container(
                container(text(self.label.as_str()).size(11).color(border_color))
                    ...
                    .style(move |_theme| iced::widget::container::Style {
                        // Upstream uses the dark green (20, 46, 30) at 0.35.
