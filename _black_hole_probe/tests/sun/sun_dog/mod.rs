...
/// Exercises an extra diamond layer ending in a third binary meld:
///
...
async fn sun_dog() {
    ...
    const FIRST_LAYER_MELDS: usize = 2;
    const FINAL_LAYER_MELDS: usize = 1;
    const MELD_TRANSFORMS: usize =
        EPOCHS * PROPAGATION_PASSES * (FIRST_LAYER_MELDS + FINAL_LAYER_MELDS);
    ...
    assert!(
        observed.len() >= MELD_TRANSFORMS,
        "expected {MELD_TRANSFORMS} meld transforms, observed {observed:?}"
    );

    let completed_epochs = &observed[..MELD_TRANSFORMS];
    ...
    assert!(
        ...
        "unexpected meld inputs in completed epochs: {completed_epochs:?}"
    );
    assert_eq!(
        completed_epochs
            .iter()
            ...
            .count(),
        EPOCHS * PROPAGATION_PASSES * FIRST_LAYER_MELDS,
        "both first-layer melds should run on every pass"
    );
    assert_eq!(
        completed_epochs
            .iter()
            ...
            .count(),
        EPOCHS * PROPAGATION_PASSES * FINAL_LAYER_MELDS,
        "the final meld should run on every pass"
    );
    ...
    for &(transform_id, p1, p2) in &observed {
        ...
        if let Some(previous_inputs) = inputs_by_transform.insert(transform_id, (p1, p2)) {
            assert_eq!(
                previous_inputs,
                (p1, p2),
                "meld transform {transform_id} did not retain a stable identity"
            );
        }
    }
    assert_eq!(
        inputs_by_transform.len(),
        FIRST_LAYER_MELDS + FINAL_LAYER_MELDS,
        "each meld journey should have a distinct transform ID"
    );
    assert_eq!(
        inputs_by_transform
            .values()
